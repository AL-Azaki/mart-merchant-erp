...
import { PurchaseReturnFormSheet } from "../components/PurchaseReturnFormSheet";

const MOCK_PURCHASE_RETURNS = [
  { id: "pret_1", return_number: "PRET-2023-0001", invoice_number: "PINV-2023-0100", date: new Date().toISOString(), total: 350.0, status: "Completed", supplier: "شركة الجملة" },
  { id: "pret_2", return_number: "PRET-2023-0002", invoice_number: "PINV-2023-0150", date: new Date(Date.now() - 86400000).toISOString(), total: 100.0, status: "Pending", supplier: "مورد التقنية" },
];
  ...
  return (
    <div style={{ padding: 24, height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h2 style={{ color: ds.textPrimary, fontSize: 20, fontWeight: 800, margin: "0 0 4px 0" }}>{isRTL ? "مرتجعات المشتريات" : "Purchase Returns"}</h2>
          <p style={{ color: ds.textSecondary, fontSize: 13, margin: 0 }}>{isRTL ? "إدارة البضائع المعادة للموردين" : "Manage returned goods to suppliers"}</p>
        </div>
        <button 
          onClick={() => { setSelectedReturn(null); setIsFormOpen(true); }}
          style={{ height: 44, background: "linear-gradient(135deg, #10B981, #059669)", border: "none", borderRadius: 12, padding: "0 16px", color: "white", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", gap: 8, cursor: "pointer", boxShadow: "0 4px 12px rgba(16, 185, 129, 0.2)" }}
        >
          <Plus size={18} strokeWidth={2.5} />
          {isRTL ? "مرتجع مشتريات جديد" : "New Purchase Return"}
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={isRTL ? "ابحث برقم الفاتورة، المرتجع، أو المورد..." : "Search by invoice, return #, or supplier..."}
      ...
      <div style={{ flex: 1, background: surface, border: `1px solid ${border}`, borderRadius: 20, overflow: "hidden" }}>
        ...
        <div style={{ overflowY: "auto", height: "calc(100% - 50px)" }}>
          {MOCK_PURCHASE_RETURNS.map((ret, idx) => {
            ...
            return (
              <div key={ret.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 1.5fr 1.5fr 1fr 1fr 1fr", gap: 16, padding: "16px 20px", borderBottom: `1px solid ${border}`, alignItems: "center" }}>
                ...
                <div style={{ fontWeight: 600, color: ds.textSecondary }}>{ret.supplier}</div>
